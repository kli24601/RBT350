...
import cv2
from reacher import color_detection
import math
...
flags.DEFINE_bool("camera"          , False, "Whether to control arms through input from camera")
...
def main(argv):
  ...
  while (True):
    ...
    x, y = color_detection.find_obj(frame, chan1, chan2)
    # if abs(prev_x - x) > 4:
    #   prev_x = x
    ...
    if time.time() - last_command > UPDATE_DT:
      ...
      if FLAGS.ik:
        ...
        xyz = [[x], [y], [10], [1]]
        C = np.array([[20, 0, 249.6, 0], [0, 20, 156.6, 0], [0,0,1,0], [0,0,0,1]])
        xyz =  np.linalg.inv(C) @ xyz
        xyz += np.expand_dims(np.array([8.5, 0, 8.5, 1]), 1)
        xyz = np.squeeze(xyz[0:3])
        xyz /= 600 # units
      ...
      if FLAGS.ik or FLAGS.camera:
          ...
          if ret is not None:
            ...
            # if np.linalg.norm(np.asarray(pos) - xyz) > 0.2:
            #   print(np.linalg.norm(np.asarray(pos) - xyz))
